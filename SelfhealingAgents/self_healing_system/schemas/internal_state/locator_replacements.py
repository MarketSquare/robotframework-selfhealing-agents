...
class LocatorReplacements(BaseModel):
    ...
    locator_origin: str = Field(
        ..., description="Origin where the locator is called (Test or Keyword)."
    )
    failed_locator: str = Field(..., description="Original locator that failed.")
    healed_locator: str = Field(
        ..., description="Healed locator that replaces the failed one."
    )

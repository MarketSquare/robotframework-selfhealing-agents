...
from RobotAid.self_healing_system.agents.browser_locator_agent import BrowserLocatorAgent
from RobotAid.self_healing_system.agents.selenium_locator_agent import SeleniumLocatorAgent
    ...
    def create_agent(
        ...
        if dom_utility is None:
            ...
            try:
                ...
                print(f"Warning: Could not create DOM utility for {agent_type.value}: {e}")
                dom_utility = None
        ...
        if agent_type == LocatorAgentType.BROWSER:
            ...
            raise ValueError(f"Unsupported agent type: {agent_type}. Supported types: {[t.value for t in LocatorAgentType if t != LocatorAgentType.APPIUM]}")
    
    @staticmethod
    def _normalize_agent_type(agent_type: Union[LocatorAgentType, str, DomUtilityType]) -> LocatorAgentType:
        ...
        if isinstance(agent_type, LocatorAgentType):
            ...
            try:
                ...
                raise ValueError(f"Unsupported agent type string: {agent_type}. Supported types: {[t.value for t in LocatorAgentType]}")
        else:

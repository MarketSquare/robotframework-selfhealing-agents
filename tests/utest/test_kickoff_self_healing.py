...
class DummyOrchestratorAgent:
    ...
    async def run_async(self, robot_ctx: dict) -> LocatorHealingResponse | str:
        return LocatorHealingResponse(suggestions=["fix1", "fix2", "fix3"])
...
def patch_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda locator_agent,
        app_settings,
        client_settings,
        usage_limits=None,
        **kwargs: DummyOrchestratorAgent(
...
def test_kickoff_healing_passes_context_and_settings() -> None:
    ...
    class SpyOrchestrator(DummyOrchestratorAgent):
        def __init__(
            self, locator_agent, app_settings, client_settings, usage_limits=None
        ) -> None:
            super().__init__(
                locator_agent=locator_agent,
                app_settings=app_settings,
